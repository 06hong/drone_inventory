...
db = SQLAlchemy() #act as database class
...
def load_user(user_id):
    return User.query.get(user_id) #when somebody login take the id and store it to load to user later
...
class User(db.Model, UserMixin):
    ...
    token = db.Column(db.String, unique= True) #related to our api
    date_created = db.Column(db.DateTime, nullable= False, default= datetime.utcnow)
    drone = db.relationship('Drone', backref = 'owner', lazy = True)
    #this class create database as well as passing in email 

    def __init__(self, email, password, token='', id=''):
        self.id= self.set_id()
        self.email = email
        self.password=self.set_password(password)
        self.token = self.set_token(24)#using secrets module

    def set_id(self):
        return str(uuid.uuid4()) #random string

    def set_password(self, password):
        self.pw_hash= generate_password_hash(password)
        return self.pw_hash #for sercutiy
    
    def set_token(self, length):
        return secrets.token_hex(length) #returns random text string of 24
...
#creating our Marshaller to pull k,v pairs out of drone
...
drones_schema = DroneSchema(many=True) #run mutiple times
...
#for field in drone_schema.Meta.fields:
    ...
    #take a python class object like <Drone x253453535>
